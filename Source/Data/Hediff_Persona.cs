...
public class Hediff_Persona : Hediff
{
    ...
    /// so the weight lands in the save file.
    /// </summary>
    ...
    /// this was written once in GetOrAddNew and never again, so a
    ...
    public static Hediff_Persona GetOrAddNew(Pawn pawn)
    {
        ...
        if (pawn.health.hediffSet.GetFirstHediffOfDef(def) is not Hediff_Persona hediff)
        {
            ...
            // in was recruit a prisoner and they stayed near-mute
    ...
    /// <summary>
    /// Drop entries that can no longer suppress anything. the key is
